...
async function like(button, userId, replyId, likeCount) {
    button.addEventListener("click", async e =>{
        ...
        const response = await fetch(`/reply/consulting-post-reply-like?userId=${userId}&replyId=${replyId}`);
...
// 게시글 좋아요 버튼
  function postLike(button,postId, userId, postLikeCount) {
    ...
    button.addEventListener("click", async e =>{
        ...
        const response = await fetch(`/post/consulting-post-like?userId=${userId}&postId=${postId}`);
