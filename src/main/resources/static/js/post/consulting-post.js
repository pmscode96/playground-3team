...
//     location.href = `/result/consulting-post-page?page=${page}&keyword=${keyword}`;
// });

$("a.change-page").on("click",async function(e){
    e.preventDefault();
    let page = $(this).attr("href");
    const keyword = document.getElementById("keyword").value;
    const response = await fetch(`/result/consulting-post-page?page=${page}&keyword=${keyword}`);
    const json = await response.json();
    console.log(json)
    if(response.ok) {
        const consultingPosts = json.consultingPosts;
        loadPage(consultingPosts);
...
function loadPageButton(data){
    ...
    // 이전 페이지 버튼
    if (data.prev) {
        paging += `<a href="${data.startPage - 1}" class="btn_paging btn_prev change-page"><span class="ico_cs ico_arr">이전 페이지로 이동</span></a>`;
    }
    // 페이지 번호 링크
    for (let i = data.startPage; i <= data.endPage; i++) {
        if (i === data.page) {
            ...
            paging += `<a href="${i}" class="link_page change-page">${i}</a>`;
        }
    }
    // 다음 페이지 버튼
    if (data.next) {
        paging += `<a href="${data.endPage + 1}" class="btn_paging btn_next change-page"><span class="ico_cs ico_arr">다음 페이지로 이동</span></a>`;
    }
